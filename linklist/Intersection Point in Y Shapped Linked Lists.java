...
class Intersect
{
    ...
    int findItntersection(int d, Node big, Node small) {
        Node bCur = big;
        Node sCur = small;
        while (d>0) {
            bCur = bCur.next;
        ...
        while (bCur != null && sCur != null) {
            if (bCur == sCur) return sCur.data;
            bCur = bCur.next;
            sCur = sCur.next;
        }
        return -1;
	...
	int intersectPoint(Node head1, Node head2)
	{
         ...
         int c2 = counts(head2);
         if (c1 < c2) {
             return findItntersection(c2-c1, head2, head1);
         } else {
             return findItntersection(c1-c2, head1, head2);
         }
	}
}
